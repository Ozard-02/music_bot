...
Three problems: (1) Qobuz enrichment returns wrong HD covers for some albums (e.g. Ditonellapiaga "Chimica"); (2) enrichment covers shouldn't override the artist's real album art; (3) when every track of an album also exists as a *single* (e.g. MadMan "MM Vol. 3"), per-track Apple ISRC enrichment returns each single's own artwork, so one album ends up with inconsistent covers. All handled by `flac_utils.resolve_cover_data(track)` — the single cover-quality resolver used by downloads (`downloader._fix_cover`) and `/fixmetadata` (`scripts/fix_metadata.py`):

1. **Spotify baseline** — the track's Spotify `cover_url` is always the album's real art; upgraded `1e02`→`b273` for 640×640 (`upgrade_cover_url`). This is the always-correct floor.
2. **HD candidates, similarity-gated** — Apple (`enrich_metadata_async(providers=["apple"])`, `cover_url_hd` bumped `100x100`→`3000x3000` via `upgrade_apple_cover`) and Deezer (`_deezer_fetch_async`, `cover_xl` 1000×1000) are tried via ISRC. A candidate only replaces the baseline if `_images_similar(candidate, baseline)` — 32×32-grid normalized mean-abs-diff `< 0.005`, decode failure → False (`_IMAGE_SIMILAR_THRESHOLD`) — **and** its area is ≥ the baseline's. Same artwork at higher res still upgrades; a different artwork never wins.
3. **Embed, never enrich-overwrite** — the chosen bytes are passed as `cover_data` into `SpotiFLAC`'s tagger (its `if not cover_data` guard means enrichment covers can never override it); after tagging, `flac_utils.embed_cover()` *replaces* the file's pictures with a single JPEG front cover — SpotiFLAC's FLAC tagger `audio.delete()` clears tags but not pictures, so without it old/wrong covers survive and duplicate PICTURE blocks pile up.
