...
47. **Docker deployment** — `Dockerfile` (python:3.14-slim + chromium + spotiflac), `.dockerignore`, `docker-compose.yml` with `~/.spotyflac` + `~/Music` mounts. `QUEUE_DB_PATH` env var redirects queue.db to the mounted volume. Containerized and bare-metal both work.
